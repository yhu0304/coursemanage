...
<html>
  ...
  <body onload="load()">
  <form action="/CourseManage/changeInfoServlet/AChangeInfo2" method="post" onsubmit="return check();">
    <table bgcolor="#cbe2e4" border="1px" cellspacing="0px"style="border-collapse:collapse">
    	...
    	<tr>
    		...
    		<td class="class2"><input id="tel" class="class4" name="telephone" type="text" value="<%=ab.getAtel()%>"/></td>
    	</tr>
    	<tr>
    		<td class="class1" style="text-align:center;">邮箱：</td>
    		<td class="class2"><input id="email" class="class4" name="email" type="text" value="<%=ab.getAemail()%>"/></td>
    	</tr>
    	<tr><td class="class3" colspan="2"><input class="class4" type="submit" name="submit" value="提交修改" align="middle" onclick="return check1('<%=ab.getAtel() %>','<%=ab.getAemail() %>')"/></td></tr>
    </table>
    </form>
    <%
